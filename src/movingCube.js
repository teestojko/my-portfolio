...
const MovingCube = () => {
    ...
    const currentSceneRef = sceneRef.current; // ローカル変数にコピー
    const scene = new THREE.Scene();
    const camera = new THREE.PerspectiveCamera(75, currentSceneRef.offsetWidth / currentSceneRef.offsetHeight, 0.1, 1000);
    camera.position.z = 15;
    ...
    // ウィンドウサイズ変更への対応
    const handleResize = () => {
        renderer.setSize(currentSceneRef.offsetWidth, currentSceneRef.offsetHeight);
        camera.aspect = currentSceneRef.offsetWidth / currentSceneRef.offsetHeight;
    ...
    return () => {
        // Cleanup
        currentSceneRef.removeChild(renderer.domElement); // DOMから削除
        renderer.dispose(); // リソース解放
